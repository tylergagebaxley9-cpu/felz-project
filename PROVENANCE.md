...
|---|---|---|---|
| `translations/*.md`, `STUDY-BOOK.md` | **OURS** | **ours** | This project's own translation, made from ORIGINAL-class sources. No published translation consulted. Judge it as you would any translator's work. **Hebrew passages** are made via `analyze.py`, which structurally cannot emit an English gloss. **Greek passages** (`john-1.md`, `revelation-3.md`, and the Greek half of `end-times.md`) have no such tool: they are made from MorphGNT parse tags plus the CNTR manuscript transcriptions, with the semantic ranges argued from the forms rather than mechanically derived. Same standard of evidence for the grammar, a **weaker** one for the lexical ranges — stated here because the two are not equivalent and the glob hides it. |
